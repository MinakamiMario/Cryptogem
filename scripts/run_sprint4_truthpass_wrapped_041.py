#!/usr/bin/env python3
"""
Sprint 4 Truth-Pass with Vol-Scale Wrapper -- Config 041

Loads the existing trade list from sprint4_041 results, applies
...
Output:
  reports/4h/sprint4_truthpass_wrapped_041.json
  reports/4h/sprint4_truthpass_wrapped_041.md

Usage:
    python3 scripts/run_sprint4_truthpass_wrapped_041.py
    python3 scripts/run_sprint4_truthpass_wrapped_041.py --resamples 5000
"""
...
# Vol-scale wrapper params (best from DDfix P1 for config 041)
...
def apply_vol_scaling(
    ...
    n_scaled = sum(1 for s in scale_values if abs(s - 1.0) > 0.01)
...
def test_window_split(trades: list[dict], windows: dict) -> dict:
    ...
    for window_name in ("early", "mid", "late"):
        ...
        window_trades = _filter_trades_by_window(trades, w["start_bar"], w["end_bar"])
        m = _compute_metrics(window_trades)
        results[window_name] = {
            "start_bar": w["start_bar"], "end_bar": w["end_bar"],
            "trades": m["trades"], "pf": m["pf"], "pnl": m["pnl"],
            "wr": m["wr"], "dd": m["dd"],
        }
    ...
    results["pass"] = windows_profitable >= 2
    return results
...
def test_walk_forward(trades: list[dict], windows: dict) -> dict:
    ...
    split_a = {
        "cal": {"start_bar": early["start_bar"], "end_bar": early["end_bar"],
                "trades": cal_a["trades"], "pf": cal_a["pf"],
                "pnl": cal_a["pnl"], "wr": cal_a["wr"], "dd": cal_a["dd"]},
        "test": {"start_bar": mid["start_bar"], "end_bar": max_bars,
    ...
    split_b = {
        "cal": {"start_bar": early["start_bar"], "end_bar": mid["end_bar"],
                "trades": cal_b["trades"], "pf": cal_b["pf"],
                "pnl": cal_b["pnl"], "wr": cal_b["wr"], "dd": cal_b["dd"]},
        "test": {"start_bar": late["start_bar"], "end_bar": max_bars,
    ...
    return {
        ...
        "pass": split_a["pass"] or split_b["pass"],
    }
...
def test_bootstrap(trade_list: list[dict], n_resamples: int = N_RESAMPLES) -> dict:
    ...
    return {
        ...
        "p5_pf": round(float(np.percentile(pfs_arr, 5)), 4),
        ...
        "pass": (float(np.percentile(pfs_arr, 5)) >= 0.85
                 and float(np.mean(pnls_arr > 0) * 100) >= 80.0),
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Sprint 4 Truth-Pass with Vol-Scale Wrapper for config 041"
    )
    ...
    print(f"\n{'=' * 72}")
    print(f"  Sprint 4 Truth-Pass (Vol-Scale Wrapped) -- Config 041")
    print(f"{'=' * 72}")
    ...
    # Write reports
    # -------------------------------------------------------------------
    json_path = output_dir / "sprint4_truthpass_wrapped_041.json"
    md_path = output_dir / "sprint4_truthpass_wrapped_041.md"
