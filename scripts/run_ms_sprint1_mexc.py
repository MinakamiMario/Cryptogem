#!/usr/bin/env python3
"""
MS Sprint 1 MEXC-Native Sweep — Market Structure Signal Screening on 4H MEXC.

Backtests 18 configs (5 families) with hybrid_notrl exits on 4H MEXC 439-coin dataset.
This is MEXC-NATIVE discovery — signals are screened directly on MEXC data,
NOT ported from Kraken (see ADR-MS-009 for rationale).
...
Usage:
    python3 scripts/run_ms_sprint1_mexc.py --dry-run
    python3 scripts/run_ms_sprint1_mexc.py --pre-count
    python3 scripts/run_ms_sprint1_mexc.py
    python3 scripts/run_ms_sprint1_mexc.py --only 1,5,12
"""
...
# ---------------------------------------------------------------------------
# Path setup — REPO must be at index 0 for strategies.4h.* imports
# ---------------------------------------------------------------------------
REPO_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(REPO_ROOT / "trading_bot"))
sys.path.insert(0, str(REPO_ROOT))  # must be last insert → index 0
...
# ---------------------------------------------------------------------------
# Constants — MEXC-native
# ---------------------------------------------------------------------------
REPORT_DIR = REPO_ROOT / "reports" / "ms"
DATASET_ID = "ohlcv_4h_mexc_spot_usdt_v2"  # 4H MEXC 439 coins
MEXC_FEE = 0.001  # 10 bps per side (MEXC spot taker)
...
def load_data():
    """Load 4H MEXC dataset and build universe."""
    path = _data_resolver.resolve_dataset(DATASET_ID)
    print(f"  Dataset: {path}")
    with open(path, "r") as f:
        data = json.load(f)
    # Count only actual coin entries (skip metadata keys)
    all_keys = [k for k in data if not k.startswith("_") and isinstance(data[k], list)]
    print(f"  Total coins in file: {len(all_keys)}")

    # Filter to coins with enough bars
    coins = [pair for pair in all_keys if len(data[pair]) >= MIN_BARS]
...
def run_pre_count(data: dict, coins: list[str], indicators: dict):
    ...
    print(f"\n{'='*72}")
    print("  MS SPRINT 1 MEXC — PRE-COUNT (structural pattern frequencies)")
    print(f"{'='*72}\n")
    ...
    estimates = {
        "A (liq_sweep)": totals["swing_lows"] // 10,
        "B (fvg_fill)": totals["fvg_bullish"] // 5,
        "C (ob_touch)": totals["ob_bullish"] // 5,
        "D (sfp)": totals["swing_lows"] // 20,
...
def run_backtest_config(config: dict, data: dict, coins: list[str], indicators: dict) -> dict:
    """Run a single config through Sprint 3 engine with DC exits and MEXC fees."""
    ...
    bt = _sprint3_engine.run_backtest(
        ...
        exit_mode="dc",
        fee=MEXC_FEE,  # 10 bps MEXC taker (was KRAKEN_FEE=26 bps)
        start_bar=START_BAR,
    )
...
def summarize_exit_classes(exit_classes: dict) -> str:
    """Format exit class distribution."""
    if not exit_classes:
        return "N/A"
    flat = {}
...
def build_scoreboard(results: list[dict]) -> str:
    ...
    lines = []
    lines.append("# MS Sprint 1 MEXC-Native — Scoreboard\n")
    lines.append(f"**Date**: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    lines.append(f"**Configs**: {len(results)}")
    lines.append(f"**Dataset**: 4H MEXC ({DATASET_ID})")
    lines.append(f"**Fees**: {MEXC_FEE*10000:.0f} bps/side (MEXC spot taker)\n")
    ...
    # Comparison with Kraken Sprint 1
    lines.append("\n## Kraken vs MEXC Comparison\n")
    lines.append("_To be filled after results. Compare family rankings and PF distributions._")
...
def main():
    parser = argparse.ArgumentParser(description="MS Sprint 1 MEXC-Native Sweep")
    ...
    if args.dry_run:
        print(f"\n{'='*72}")
        print("  MS SPRINT 1 MEXC — DRY RUN ({} configs)".format(len(configs)))
        print(f"{'='*72}\n")
        for c in configs:
        ...
        print(f"  Fees: {MEXC_FEE*10000:.0f} bps/side")
        return

    # --- Load data ---
    print(f"\n{'='*72}")
    print("  MS SPRINT 1 MEXC — Loading data")
    print(f"{'='*72}\n")
    ...
    scoreboard_json = REPORT_DIR / "sprint1_mexc_scoreboard.json"
    if scoreboard_json.exists() and not args.force:
        print(f"\n  Results already exist at {scoreboard_json}")
        print(f"  Use --force to overwrite")
        return

    # --- Run backtests ---
    print(f"\n{'='*72}")
    print(f"  MS SPRINT 1 MEXC — Running {len(configs)} backtests")
    print(f"  Dataset: {DATASET_ID} | Fee: {MEXC_FEE*10000:.0f} bps/side")
    print(f"{'='*72}\n")
    ...
    print(f"  Dataset: {DATASET_ID}")
    print(f"  Fees: {MEXC_FEE*10000:.0f} bps/side (MEXC spot)")
    ...
    # Markdown scoreboard
    scoreboard_md = REPORT_DIR / "sprint1_mexc_scoreboard.md"
