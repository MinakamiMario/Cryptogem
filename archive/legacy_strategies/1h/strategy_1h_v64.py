"""
V64: 1H ROC Momentum (Rate of Change + EMA Trend)
---------------------------------------------------
Rate of Change (ROC) measures % price change over N periods.
Positive ROC = momentum is up. Use as entry confirmation.

Entry:
1. Triple EMA aligned (20>50>100) + EMA20 rising
2. ROC(12) > 0 (positive momentum over last 12 bars)
3. ROC(12) > ROC(12) 1 bar ago (momentum accelerating)
4. RSI 45-75
5. ADX > 20
...
class ROCMomentum1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
    ...
    def next(self):
        ...
        roc_rising = self.roc[0] > self.roc[-1]
