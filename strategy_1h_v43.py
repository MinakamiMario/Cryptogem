"""
V43: 1H Ultra-Wide Momentum Rider
------------------------------------
The pattern from all 1H tests:
- Wider stops = better results (5x ATR is sweet spot)
- Fewer exits = better (only trailing stop works)
- Trend confirmation essential
- Vol filter prevents crash entries

Radical simplicity approach:
1. Entry: EMA20 > EMA50 > EMA100 (strong triple alignment)
   + Close > EMA20 (price above fast EMA)
   + EMA20 rising (slope positive)
   + RSI 45-75 (momentum but not overbought)
   + No vol spike
2. Exit: ONLY 6x ATR trailing stop
   No EMA cross exit, no other exits
3. Ultra-wide stop lets BTC breathe through normal swings
...
class UltraWideMomentum1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
