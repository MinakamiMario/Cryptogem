#!/usr/bin/env python3
"""
Part 2 Robustness Sweep on FULL 316-coin universe (T1=100, T2=216)
===================================================================
Re-runs the same 12-variant neighborhood grid from run_h20_robustness.py
but on ALL available tier coins instead of the partial 135-coin download.

Key additions over h20_robustness_002:
  - Full 316-coin universe (was 135)
  - G7 gate assessment: neighbor stability >= 8/12 profitable
  - Stress survival count
  - ADR HF-031 compatibility

Output:
  reports/hf/part2_robustness_316_001.json
  reports/hf/part2_robustness_316_001.md

Usage:
    python -m strategies.hf.screening.run_part2_robustness_316
    python -m strategies.hf.screening.run_part2_robustness_316 --dry-run
    python -m strategies.hf.screening.run_part2_robustness_316 --require-data
"""
import sys
import json
import time
...
REPORT_JSON = 'part2_robustness_316_001.json'
REPORT_MD   = 'part2_robustness_316_001.md'


def main():
    parser = argparse.ArgumentParser(
        description='Part 2 Robustness Sweep - Full 316-coin Universe',
    )
    ...
    print('=' * 70)
    print('  Part 2 Robustness Sweep - Full 316-coin Universe')
    ...
    n_total = n_t1 + n_t2
    print('[Universe] T1: %d coins, T2: %d coins, Total: %d' % (n_t1, n_t2, n_total))

    if n_total < 200:
        print('[WARN] Expected ~316 coins but only %d available.' % n_total)
    ...
    report = {
        'run_header': {
            ...
            'universe': 'T1(%d)+T2(%d)' % (n_t1, n_t2),
            'universe_total': n_total,
            'previous_run_universe': 'T1(43)+T2(92) = 135',
            'timeframe': '1h',
    ...
    # ============================================================
    md = []
    md.append('# Part 2 Robustness Sweep - Full 316-coin Universe')
    md.append('')
    md.append('**Date**: %s' % datetime.now().strftime('%Y-%m-%d %H:%M'))
    md.append('**Commit**: %s' % commit)
    md.append('**Universe**: T1(%d) + T2(%d) = %d coins' % (n_t1, n_t2, n_total))
    md.append('**Previous run**: T1(43) + T2(92) = 135 coins (h20_robustness_002)')
    ...
    # Comparison note
    md.append('## Comparison: 316 vs 135 coins')
    md.append('')
    md.append('This run uses the full T1(100)+T2(216) universe vs the previous')
    md.append('partial T1(43)+T2(92) download. Key differences to watch:')
    md.append('- More T2 coins means higher fee drag (T2=23.5bps vs T1=12.5bps)')
    md.append('- More coins = more trade opportunities but also more noise')
    md.append('- G7 threshold unchanged: >= 8/12 profitable neighbors')
    md.append('')

    # Footer
    md.append('---')
    md.append('*Generated by strategies/hf/screening/run_part2_robustness_316.py at %s*' % (
    ...
    # Final summary
    print('\n' + '=' * 70)
    print('  COMPLETE: %d variants on %d coins' % (len(ROBUSTNESS_GRID), n_total))
