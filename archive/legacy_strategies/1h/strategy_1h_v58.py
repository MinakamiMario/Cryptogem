"""
V58: 1H CCI Momentum + ADX (Commodity Channel Index)
-------------------------------------------------------
CCI measures price deviation from its statistical mean.
CCI > 100 = strong upward trend. CCI 0-100 = building momentum.

Entry:
1. Triple EMA aligned (20>50>100) + EMA20 rising
2. CCI between 50 and 200 (strong momentum, not extreme)
3. ADX > 20
...
class CCIMomentum1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
    ...
    def next(self):
        ...
        if not self.position:
            if (not in_cooldown and
                    ...
                    self.p.cci_min < self.cci[0] < self.p.cci_max and
