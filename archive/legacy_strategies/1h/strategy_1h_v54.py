"""
V54: 1H Volume Trend + ADX (Volume Rising Confirmation)
----------------------------------------------------------
Use volume EMA trend to confirm buying pressure.
Volume EMA(5) > Volume SMA(20) = increasing participation.

Entry:
1. Triple EMA aligned (20>50>100) + EMA20 rising
2. RSI 45-75
3. ADX > 20
4. Volume EMA(5) > Volume SMA(20) = volume trend rising
...
class VolumeTrend1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
    ...
    def __init__(self):
        ...
        self.vol_sma = bt.indicators.SMA(self.data.volume, period=self.p.vol_trend_period)
        self.vol_ema_fast = bt.indicators.EMA(self.data.volume, period=5)
    ...
    def next(self):
        ...
        # Volume rising: fast vol EMA > slow vol SMA
        vol_rising = self.vol_ema_fast[0] > self.vol_sma[0] if self.vol_sma[0] > 0 else False
