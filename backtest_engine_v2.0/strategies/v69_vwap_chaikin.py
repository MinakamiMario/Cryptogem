"""
V69: 1H VWAP + Chaikin Hybrid (Best Risk-Adjusted: PF 1.35, MaxDD 20.2%)
---------------------------------------------------------------------------
Entry:
1. Triple EMA aligned (20>50>100) + EMA20 rising
2. Price > VWMA(20) (V55's volume-weighted confirmation)
3. ATR(10) rising for 2 bars (V65's volatility expansion)
4. Close > Close 3 bars ago (price momentum confirms)
5. RSI 45-75
6. ADX > 20
7. Vol filter + cooldown
...
class VWAPChaikin1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
        ('vwma_period', 20),
        ('chaikin_atr_period', 10),
        ('momentum_lookback', 3),
        ('rsi_period', 14),
        ('rsi_min', 45),
        ('rsi_max', 75),
    ...
    def __init__(self):
        ...
        self.vol_sma_ind = bt.indicators.SMA(self.data.volume, period=self.p.vwma_period)
        self.rsi = bt.indicators.RSI(self.data.close, period=self.p.rsi_period)
        self.adx = bt.indicators.AverageDirectionalMovementIndex(
            self.data, period=self.p.adx_period)
        self.chaikin_atr = bt.indicators.ATR(self.data, period=self.p.chaikin_atr_period)
    ...
    def get_vwma(self):
        if self.vol_sma_ind[0] > 0:
            return self.pv_sma[0] / self.vol_sma_ind[0]
    ...
    def next(self):
        ...
        # Chaikin volatility expansion
        atr_expanding = (self.chaikin_atr[0] > self.chaikin_atr[-1] and
                         self.chaikin_atr[-1] > self.chaikin_atr[-2])
        price_momentum = self.data.close[0] > self.data.close[-self.p.momentum_lookback]
