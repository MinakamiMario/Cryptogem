"""
V57: 1H Supertrend + ADX + Ultra-Wide Stop
---------------------------------------------
Supertrend is a popular trend indicator that combines ATR with direction.
It flips bullish/bearish and provides clean signals.

We build a manual Supertrend (since backtrader doesn't have built-in):
- Supertrend = price above (HL2 - multiplier * ATR) = bullish
- Combined with our proven V49 framework

Entry:
1. Supertrend bullish (price above lower band)
2. Triple EMA aligned (20>50>100) + EMA20 rising
3. RSI 45-75
4. ADX > 20
...
class SupertrendMomentum1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
    ...
    def __init__(self):
        ...
        self.st_upper = 0
        self.st_lower = 0
        self.st_trend = 1  # 1 = bullish, -1 = bearish
    ...
    def next(self):
        ...
        # Final bands (only move in favorable direction)
