#!/usr/bin/env python3
"""
Sprint 4 Truth-Pass — Robustness validation for top-5 DC-compatible entries.
...
Usage:
    python3 scripts/run_sprint4_truthpass.py
    python3 scripts/run_sprint4_truthpass.py --only 041,032
    python3 scripts/run_sprint4_truthpass.py --resamples 5000
    python3 scripts/run_sprint4_truthpass.py --universe strategies/4h/universe_sprint1.json
"""
...
# Top-5 configs from Sprint 4 sweep (by PF)
TOP5_PATTERNS = [
...
def _write_md_report(result: dict, output_path: Path) -> None:
    ...
    r = result
    lines = [
        f"# Sprint 4 Truth-Pass: {r['config_id']}",
        "",
...
def _write_summary(all_results: list[dict], output_dir: Path) -> None:
    ...
    json_path = output_dir / "sprint4_truthpass_summary.json"
    ...
    md_path = output_dir / "sprint4_truthpass_summary.md"
...
def main():
    parser = argparse.ArgumentParser(
        description="Sprint 4 Truth-Pass: robustness validation for top DC-compatible entries"
    )
    parser.add_argument("--dataset", default=DEFAULT_DATASET,
                        help="Dataset ID or alias (default: ohlcv_4h_kraken_spot_usd_526)")
    parser.add_argument("--universe", default=str(REPO_ROOT / "strategies" / "4h" / "universe_sprint1.json"),
                        help="Universe JSON path")
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT),
                        help="Output directory for reports")
    parser.add_argument("--only", default=None,
                        help="Comma-separated 3-digit suffixes to filter configs (e.g. 041,032)")
    ...
    print(f"\n{'=' * 72}")
    print(f"  Sprint 4 Truth-Pass -- Robustness Validation")
    print(f"{'=' * 72}")
    ...
    if args.only:
        # Filter by 3-digit suffix patterns (e.g. "041,032")
    ...
    coins = universe_data["coins"] if isinstance(universe_data, dict) else universe_data
    ...
    for ci, cfg in enumerate(configs_to_test, 1):
        ...
        json_path = output_dir / f"sprint4_truthpass_{config_id}.json"
        md_path = output_dir / f"sprint4_truthpass_{config_id}.md"
