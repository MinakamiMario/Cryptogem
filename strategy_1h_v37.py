"""
V37: 1H Hybrid Keltner Swing (Best of V30 + V35)
---------------------------------------------------
Combine the two approaches that showed promise:
- V30 Keltner wide breakout (only positive return)
- V35 EMA trend filter (proven on 1D)

Entry: Keltner breakout (2.0 ATR) + EMA50 > EMA100 (trend confirmed)
       + RSI 50-75 + no vol spike
Exit: EMA50 death cross below EMA100 OR trailing stop OR mid-channel
Hold: No max hold - let winners run with trailing stop
...
class HybridKeltnerSwing1H(bt.Strategy):
    params = (
        ('kc_period', 20),
        ('kc_atr_mult', 2.0),
        ('fast_ema', 50),
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # Exit: EMA death cross, mid-channel, or trailing stop
            if self.ema_cross < 0:
                self.order = self.sell(size=self.position.size)
                self.last_exit_bar = len(self)
            elif self.data.close[0] < self.kc_mid[0]:
                self.order = self.sell(size=self.position.size)
                self.last_exit_bar = len(self)
