#!/usr/bin/env python3
"""
Part 2 Robustness Sweep on 295-coin universe (excl_all_negative)
================================================================
Same 12-variant ROBUSTNESS_GRID as run_h20_robustness.py and
run_part2_robustness_316.py, but on the 295-coin universe that
excludes the 21 net-negative coins identified by the loss cluster
analysis (Agent A3).

Excluded coins (21):
  AI3/USD, ALKIMI/USD, ANIME/USD, CFG/USD, DBR/USD, ESX/USD,
  GST/USD, HOUSE/USD, KET/USD, LMWR/USD, MXC/USD, ODOS/USD,
  PERP/USD, PNUT/USD, POLIS/USD, RARI/USD, SUKU/USD, TANSSI/USD,
  TITCOIN/USD, TOSHI/USD, WMTX/USD

Key differences vs 316-coin run:
  - 21 net-negative coins removed (loss cluster analysis)
  - Same G7 gate: >= 8/12 variants profitable (PF > 1.0)
  - Same 3 passes: baseline, walk-forward 5-fold, stress 2x

Output:
  reports/hf/part2_robustness_295_001.json
  reports/hf/part2_robustness_295_001.md

Usage:
    python -m strategies.hf.screening.run_part2_robustness_295
    python -m strategies.hf.screening.run_part2_robustness_295 --dry-run
    python -m strategies.hf.screening.run_part2_robustness_295 --require-data
"""
import sys
import json
import time
...
REPORT_JSON = 'part2_robustness_295_001.json'
REPORT_MD   = 'part2_robustness_295_001.md'

# 21 net-negative coins identified by loss cluster analysis (Agent A3)
# Source: reports/hf/part2_loss_cluster_001.json -> excl_all_negative
EXCLUDED_COINS = {
...
def load_excluded_coins_from_report():
    """Load excluded coins from the loss cluster report JSON if available."""
    report_path = ROOT / 'reports' / 'hf' / 'part2_loss_cluster_001.json'
    if report_path.exists():
        with open(report_path) as f:
            report = json.load(f)
        for result in report.get('exclusion_results', []):
            if result.get('label') == 'excl_all_negative':
...
def main():
    parser = argparse.ArgumentParser(
        description='Part 2 Robustness Sweep - 295-coin Universe (excl_all_negative)',
    )
    ...
    print('=' * 70)
    print('  Part 2 Robustness Sweep - 295-coin Universe (excl_all_negative)')
    print('  Cost Regime: MEXC Market (costs_mexc_v2)')
    print('  Grid: 12 variants (same as h20_robustness_002)')
    print('  Exclusion: 21 net-negative coins from loss cluster analysis')
    print('=' * 70)
    ...
    if excl_from_report is not None:
        ...
        excl_source = 'part2_loss_cluster_001.json'
    ...
    if n_total < 200:
        print('[WARN] Expected ~295 coins but only %d available.' % n_total)
    ...
    report = {
        'run_header': {
            ...
            'universe': 'T1(%d)+T2(%d) [excl_all_negative]' % (n_t1, n_t2),
            ...
            'previous_run': 'part2_robustness_316 (full universe, G7=9/12 PASS)',
            'timeframe': '1h',
    ...
    # ============================================================
    md = []
    md.append('# Part 2 Robustness Sweep - 295-coin Universe (excl_all_negative)')
    md.append('')
    ...
    # Comparison section
    md.append('## Comparison: 295 vs 316 coins')
    md.append('')
    md.append('| Metric | 316 coins | 295 coins | Delta |')
    md.append('|--------|-----------|-----------|-------|')
    md.append('| G7 profitable | 9/12 | %d/12 | %+d |' % (
        n_profitable, n_profitable - 9))
    md.append('| Stress profitable | - | %d/12 | - |' % n_stress_profitable)
    md.append('| WF >= 3/5 | - | %d/12 | - |' % n_wf_3plus)
    md.append('')
    md.append('**Note**: 316-coin G7 was 9/12 PASS (from part2_robustness_316_001).')
    md.append('Excluding 21 net-negative coins should improve edge across all variants.')
    md.append('')
    ...
    # Footer
    md.append('---')
    md.append('*Generated by strategies/hf/screening/run_part2_robustness_295.py at %s*' % (
    ...
    # Final summary
    print('\n' + '=' * 70)
    print('  COMPLETE: %d variants on %d coins (excl %d net-negative)' % (
    ...
    print('  Comparison: 316-coin G7 was 9/12 PASS')
    print('=' * 70)
