#!/usr/bin/env python3
"""
Sprint 2 Sweep Runner — Stage 0 entry-edge discovery for 4H strategies.

Runs 4 signal families (24 configs) through the Sprint 1 screening engine
with Sprint 2 extended indicators and cross-sectional market context.

Key differences from Sprint 1 runner:
  - Uses Sprint 2 indicators (dc_prev_high, plus_di, minus_di, __coin__)
  - Precomputes market context for Family 3 (H4S-03: relative strength)
  - Injects __market__ into params for all configs
  - Uses relaxed Sprint 2 gates (PF > 1.05 advancement)

Usage:
    python3 scripts/run_sprint2_sweep.py
    python3 scripts/run_sprint2_sweep.py --dry-run
    python3 scripts/run_sprint2_sweep.py --force
    python3 scripts/run_sprint2_sweep.py --only 1,5,12
    python3 scripts/run_sprint2_sweep.py --universe strategies/4h/universe_sprint1.json
"""
...
_sprint2_hyp = importlib.import_module("strategies.4h.sprint2.hypotheses")
...
precompute_all = _sprint2_indicators.precompute_all  # Sprint 2 extended
...
build_sweep_configs = _sprint2_hyp.build_sweep_configs
...
def _run_single(
    ...
    # Inject __market__ into params (no-op for families that don't use it)
    enriched_params = {**cfg["params"], "__market__": market_ctx}

    # Determine momentum_period for this config (if Family 3, use its period)
    # This doesn't affect the precomputed context since it's already done
    # but we note it in metadata for provenance
    ...
    # Evaluate Sprint 2 gates (relaxed)
    bt_dict = {
        "trades": bt.trades,
        "pnl": bt.pnl,
        "pf": bt.pf,
    ...
    meta = {
        ...
        "git_hash": git_hash,
        "engine": "sprint1.engine (Stage 0) + sprint2 indicators/context",
        "market_context": True,
...
def _build_scoreboard(results: list[dict], output_dir: Path, git_hash: str, dataset_id: str):
    ...
    sb_json = output_dir / "scoreboard_sprint2.json"
    ...
    sb_md = output_dir / "scoreboard_sprint2.md"
...
def main():
    parser = argparse.ArgumentParser(description="Sprint 2 Sweep Runner (Stage 0 — Entry-Edge Discovery)")
    ...
    print(f"\n{'=' * 72}")
    print(f"  Sprint 2 Sweep — Stage 0 Entry-Edge Discovery")
    print(f"{'=' * 72}")
    ...
    # Sanity: equity should never go negative
    # (Verified by Sprint 1 engine fix: equity < 0 breaks, not equity <= 0)

    # Run sweep
    print(f"\n{'─' * 72}")
    print(f"  Running {len(configs)} configs (Stage 0, PF > {_sprint2_gates.STAGE0_MIN_PF} advancement)...")
    print(f"{'─' * 72}")
