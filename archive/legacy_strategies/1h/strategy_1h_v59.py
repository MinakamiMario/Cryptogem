"""
V59: 1H Donchian Channel Breakout + ADX
------------------------------------------
Donchian Channels = highest high / lowest low over N bars.
Classic trend breakout system used by the "Turtle Traders".

Entry:
1. Price breaks above 48h Donchian upper (2-day high breakout)
2. Triple EMA aligned (20>50>100) + EMA20 rising
3. ADX > 20 (trend confirmed)
4. Vol filter + cooldown
...
class DonchianBreakout1H(bt.Strategy):
    params = (
        ('fast_period', 20),
        ('mid_period', 50),
        ('slow_period', 100),
    ...
    def next(self):
        ...
        # Donchian breakout: close equals or exceeds the highest high
        donchian_break = self.data.close[0] >= self.highest_high[-1]
